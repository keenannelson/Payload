...
//This version focuses on file control. It takes an image from cap and then puts it in crop and also takes the cap image and puts it in donel
//It uses a relative pathing system

//The previous version this is built off of is FrequencySaliencyDetection8
...
int main(int argc, char ** argv)
{
	...
	InputImage << "./cap/" << argv[1];

	//Mat OriginalImage = imread("C:/Users/KeenanAsus/Documents/ucr_uav/targets/IMG_0111.jpg"); 
	//Mat OriginalImage = imread("./cap/IMG_0111.jpg");
	Mat OriginalImage = imread(InputImage.str());//reads the passed in image from main

	if (!OriginalImage.data)
	{
		printf("No image data. Check to see if there is a cap folder. \n");
		return -1;
	}
	...
	//system("mkdir crop");//system("mkdir -p /tmp"); C:\\NewDir
	//ostringstream FileName;
	//int i = 26;
	//FileName << "./crop/RegionOfInterest" << i << ".png";//Create file name
	//imwrite(FileName.str(), HSVplane);//writing file
...
void CropTargets(Mat imgThresholded, Mat OriginalImage, int SizeReduced) {
	...
	for (int i = 0; i < contours.size(); i++) {
		...
		int CaptureArea = 100;//size of cropped image
	...
	system("mkdir crop");//make crop folder if it doesn't exist
	for (int i = 0; i < (index); i++) {
		ostringstream FileName;
		FileName << "./crop/RegionOfInterest" << i << ".png";//Create file name
		imwrite(FileName.str(), ROI[i]);//writing file
										//namedWindow(ROIString, WINDOW_AUTOSIZE);
										//imshow(ROIString, ROI[i]);
	}
	/// Show in a window
	//("Contours", WINDOW_AUTOSIZE);
